...
namespace HTTPServer::Core
{
  ...
  HttpResponse::HttpResponse(HttpStatus status, String &&body, HttpHeadersCollection headers)
      : status_(status), headers_(std::move(headers)), body_(HttpResponseBodyStream::create<StringStream>(std::move(body)))
  {
    init(body_->available());
  }

  HttpResponse::HttpResponse(HttpStatus status, const String &body, HttpHeadersCollection headers)
      : status_(status), headers_(std::move(headers)), body_(HttpResponseBodyStream::create<StringStream>(body))
  {
    init(body_->available());
  }
  ...
  HttpResponse::HttpResponse(HttpStatus status, const char *body, HttpHeadersCollection headers)
      : status_(status), headers_(std::move(headers)), body_(HttpResponseBodyStream::create<OctetsStream>(body))
  {
    init(body_->available());
  }
  ...
  void ChunkedHttpResponseBodyStream::bufferNextChunk()
  {
    ...
    size_t dataOffset = hex_length(HTTPServer::CHUNKED_RESPONSE_BUFFER_SIZE) + 3;          // +3 for \r\n and null terminator
    size_t maxInnerReadLength = HTTPServer::CHUNKED_RESPONSE_BUFFER_SIZE - dataOffset - 2; // -2 for trailing CRLF
    size_t bytesRead = 0;
