...
namespace HTTPServer::StaticFiles
{
    ...
    {
    private:
        HTTPServer::Core::HttpContentTypes &contentTypes_;
    ...
    public:
        StaticFileHandlerFactory(FileLocator &fileLocator, HTTPServer::Core::HttpContentTypes &contentTypes) : fileLocator_(fileLocator), contentTypes_(contentTypes)
        {
        }
        ...
        std::unique_ptr<IHttpHandler> create(HttpContext &context) override
        {
            ...
            return std::make_unique<HttpHandler>(
                ...
                HttpHeaders::ETag(getEtag(file)),
                HttpHeaders::LastModified(getLastWriteValue(file)),
