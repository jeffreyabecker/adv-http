...
namespace
{
    ...
    {
        const auto params = lumalink::http::util::WebUtility::ParseQueryParameters("name=Jane+Doe&role=admin&empty", strlen("name=Jane+Doe&role=admin&empty"));
    ...
    void test_web_utility_std_string_view_overloads_remain_available()
    {
        ...
        const auto pairs = lumalink::http::util::WebUtility::ParseQueryString(query);
        ...
        const std::string encoded = lumalink::http::util::WebUtility::HtmlEncode(std::string_view("<&>\"'"));
        TEST_ASSERT_EQUAL_STRING("&lt;&amp;&gt;&quot;&#39;", encoded.c_str());

        const std::string decoded = lumalink::http::util::WebUtility::DecodeURIComponent(std::string_view("Jane%20Doe"));
        TEST_ASSERT_EQUAL_STRING("Jane Doe", decoded.c_str());
    }

    void test_web_utility_base64_decode_to_std_string()
    {
        const std::string decoded = lumalink::http::util::WebUtility::Base64DecodeToString(std::string_view("SmFuZSBEb2U="));
    ...
    {
        const std::vector<uint8_t> decoded = lumalink::http::util::WebUtility::Base64Decode(std::string_view("!!!!"));
    ...
    {
        const std::string encodedFromView = lumalink::http::util::WebUtility::EncodeURIComponent(std::string_view("one two/three?four=five&six"));
        const std::string encodedFromChars = lumalink::http::util::WebUtility::EncodeURIComponent("one two/three?four=five&six", strlen("one two/three?four=five&six"));
    ...
    {
        const std::string validDecoded = lumalink::http::util::WebUtility::DecodeURIComponent(std::string_view("alpha%2Fbeta+gamma"));
        const std::string malformedHexDecoded = lumalink::http::util::WebUtility::DecodeURIComponent(std::string_view("%ZZ"));
        const std::string partialPercentDecoded = lumalink::http::util::WebUtility::DecodeURIComponent(std::string_view("trail%"));
        const std::string partialNibbleDecoded = lumalink::http::util::WebUtility::DecodeURIComponent(std::string_view("trail%A"));
    ...
    void test_parse_query_parameters_freezes_empty_repeated_and_plus_decoded_values()
    {
        ...
        const auto params = lumalink::http::util::WebUtility::ParseQueryParameters(query);
    ...
    {
        const lumalink::http::util::UriView uri("https://user@example.com:8443/path/to/resource?first=one&second=two#frag");
    ...
    {
        const lumalink::http::util::UriView relativeUri("/docs/page?mode=edit#section-2");
        ...
        const lumalink::http::util::UriView queryOnlyUri("?first=one&first=two");
        ...
        const lumalink::http::util::UriView fragmentOnlyUri("#frag-only");
        ...
        const lumalink::http::util::UriView authorityLessUri("file:/tmp/data.json?download=true#anchor");
    ...
    void test_header_collection_freezes_merge_overwrite_and_set_cookie_semantics()
    {
        ...
        headers.set(lumalink::http::core::HttpHeader::SetCookie("a=1"), false);
        headers.set(lumalink::http::core::HttpHeader::SetCookie("b=2"), false);
    ...
    {
        const lumalink::http::core::HttpHeader header(std::string("X-Mode"), std::string("native"));
    ...
    {
        const lumalink::http::core::HttpHeader emptyHeader{std::string(), std::string()};
        TEST_ASSERT_TRUE(emptyHeader.nameView().empty());
        TEST_ASSERT_TRUE(emptyHeader.valueView().empty());

        const lumalink::http::core::HttpHeader copiedHeader(emptyHeader);
        TEST_ASSERT_TRUE(copiedHeader.nameView().empty());
        TEST_ASSERT_TRUE(copiedHeader.valueView().empty());

        lumalink::http::core::HttpHeader movedHeader(lumalink::http::core::HttpHeader("X-Trace", "abc123"));
    ...
    {
        const lumalink::http::core::HttpStatus ok = lumalink::http::core::HttpStatus::Ok();
        const lumalink::http::core::HttpStatus custom = lumalink::http::core::HttpStatus(299, "Custom Success");

        TEST_ASSERT_EQUAL_UINT16(200, ok.code());
        TEST_ASSERT_EQUAL_STRING("OK", ok.toString());
        TEST_ASSERT_TRUE(ok == static_cast<uint16_t>(200));
        TEST_ASSERT_TRUE(lumalink::http::core::HttpStatus::BadRequest() < lumalink::http::core::HttpStatus::InternalServerError());
    ...
    {
        TEST_ASSERT_TRUE(lumalink::http::core::HttpMethod::isRequestBodyAllowed(lumalink::http::core::HttpMethod::Post));
        TEST_ASSERT_TRUE(lumalink::http::core::HttpMethod::isRequestBodyAllowed(lumalink::http::core::HttpMethod::Put));
        TEST_ASSERT_TRUE(lumalink::http::core::HttpMethod::isRequestBodyAllowed(lumalink::http::core::HttpMethod::Patch));
        TEST_ASSERT_TRUE(lumalink::http::core::HttpMethod::isRequestBodyAllowed(lumalink::http::core::HttpMethod::Delete));

        TEST_ASSERT_FALSE(lumalink::http::core::HttpMethod::isRequestBodyAllowed(lumalink::http::core::HttpMethod::Get));
        TEST_ASSERT_FALSE(lumalink::http::core::HttpMethod::isRequestBodyAllowed(lumalink::http::core::HttpMethod::Head));
        TEST_ASSERT_FALSE(lumalink::http::core::HttpMethod::isRequestBodyAllowed(lumalink::http::core::HttpMethod::Options));
        TEST_ASSERT_FALSE(lumalink::http::core::HttpMethod::isRequestBodyAllowed("CUSTOM"));
    }
    ...
    void test_http_content_types_normalizes_extensions_and_falls_back_to_unknown()
    {
        ...
        TEST_ASSERT_EQUAL_STRING(lumalink::http::core::HttpContentTypes::Json, contentTypes.getContentTypeByExtension("JSON"));
        TEST_ASSERT_EQUAL_STRING(lumalink::http::core::HttpContentTypes::Svg, contentTypes.getContentTypeFromPath("/assets/icon.SVG"));
        TEST_ASSERT_EQUAL_STRING(lumalink::http::core::HttpContentTypes::Unknown, contentTypes.getContentTypeFromPath("/assets/README"));
    ...
    {
        const std::string encodedNull = lumalink::http::util::WebUtility::HtmlEncode(static_cast<const char *>(nullptr));
        const std::string encodedEmpty = lumalink::http::util::WebUtility::HtmlEncode("", 0);
    ...
    {
        const std::string encoded = lumalink::http::util::WebUtility::HtmlEncode("<&>\"'", 5);
    ...
    {
        const std::string encodedNull = lumalink::http::util::WebUtility::HtmlAttributeEncode(static_cast<const char *>(nullptr));
        const std::string encodedEmpty = lumalink::http::util::WebUtility::HtmlAttributeEncode("", 0);
    ...
    {
        const std::string encoded = lumalink::http::util::WebUtility::HtmlAttributeEncode("<&>\"'", 5);
    ...
    {
        const std::string encodedWithQuotes = lumalink::http::util::WebUtility::JavaScriptStringEncode(std::string_view("\"\\\n\t\x01"));
        const std::string encodedWithoutQuotes = lumalink::http::util::WebUtility::JavaScriptStringEncode(std::string_view("plain"), false);
        const std::string encodedEmptyWithQuotes = lumalink::http::util::WebUtility::JavaScriptStringEncode(std::string_view(), true);
        const std::string encodedEmptyWithoutQuotes = lumalink::http::util::WebUtility::JavaScriptStringEncode(std::string_view(), false);
    ...
    {
        std::string body("ok");
        auto response = lumalink::http::response::StringResponse::create(
            lumalink::http::core::HttpStatus::Ok(),
            std::move(body),
            std::initializer_list<lumalink::http::core::HttpHeader>{});
        auto filter = lumalink::http::routing::CrossOriginRequestSharing(
        ...
        const auto allowOrigin = response->headers().find(std::string_view(lumalink::http::core::HttpHeaderNames::AccessControlAllowOrigin));
        TEST_ASSERT_TRUE(allowOrigin.has_value());
        TEST_ASSERT_EQUAL_STRING("https://example.com", std::string(allowOrigin->valueView()).c_str());

        const auto maxAge = response->headers().find(std::string_view(lumalink::http::core::HttpHeaderNames::AccessControlMaxAge));
    ...
    void test_request_handler_factory_std_text_overloads_delegate_to_std_string()
    {
        ...
        factory.createResponse(lumalink::http::core::HttpStatus::Ok(), std::string_view("first"));
        TEST_ASSERT_EQUAL_STRING("first", factory.lastResponseBody().c_str());

        const std::string second("second");
        factory.createResponse(lumalink::http::core::HttpStatus::Ok(), second);
        TEST_ASSERT_EQUAL_STRING("second", factory.lastResponseBody().c_str());

        factory.createResponse(lumalink::http::core::HttpStatus::Ok(), "third");
        TEST_ASSERT_EQUAL_STRING("third", factory.lastResponseBody().c_str());
    }

    void test_http_context_items_exposes_std_string_keyed_map()
    {
        using ItemsType = std::remove_reference_t<decltype(std::declval<const lumalink::http::core::HttpContext &>().items())>;
    ...
    {
        lumalink::http::routing::HandlerMatcher matcher(std::string_view("/files/:fileName"), std::string_view("get,post"), {std::string_view("Application/Json"), std::string_view("text/plain")});

        TEST_ASSERT_EQUAL_STRING("/files/:fileName", std::string(matcher.getUriPattern()).c_str());
        ...
        matcher.setUriPattern(std::string_view("/assets/:assetName"));
        matcher.setAllowedMethods(std::string_view("head"));
        matcher.setAllowedContentTypes({std::string_view("IMAGE/SVG+XML")});

        TEST_ASSERT_EQUAL_STRING("/assets/:assetName", std::string(matcher.getUriPattern()).c_str());
    ...
    void test_handler_matcher_default_helpers_use_standard_text_inputs()
    {
        ...
        TEST_ASSERT_TRUE(lumalink::http::routing::defaultCheckMethod(std::string_view("GET,POST"), std::string_view("GET")));
        TEST_ASSERT_FALSE(lumalink::http::routing::defaultCheckMethod(std::string_view("GET,POST"), std::string_view("PUT")));

        TEST_ASSERT_TRUE(lumalink::http::routing::defaultCheckUriPattern(std::string_view("/files/report.txt"), wildcardPattern));
        TEST_ASSERT_FALSE(lumalink::http::routing::defaultCheckUriPattern(std::string_view("/files/report.txt"), std::string_view("/assets/:assetName")));
        TEST_ASSERT_TRUE(lumalink::http::routing::defaultCheckUriPattern(std::string_view("/docs/site/index.html"), htmlGlobPattern));
        TEST_ASSERT_FALSE(lumalink::http::routing::defaultCheckUriPattern(std::string_view("/docs/site/index.css"), htmlGlobPattern));
    }
