...
namespace HttpServerAdvanced::TestSupport
{
    ...
    inline std::string DrainByteSourceWithAvailability(IByteSource &source, std::size_t maxUnavailablePolls = 8)
    {
        ...
        while (true)
        {
            ...
            {
                const std::size_t bytesRead = source.read(HttpServerAdvanced::span<std::uint8_t>(buffer, (std::min)(available.count, sizeof(buffer))));
                if (bytesRead == 0)
                {
                    break;
                }
    ...
    private:
        ...
        std::size_t copyInto(HttpServerAdvanced::span<std::uint8_t> buffer, bool consume)
        {
            advancePastConsumedSteps();
