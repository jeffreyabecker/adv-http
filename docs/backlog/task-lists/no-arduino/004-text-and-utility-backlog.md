2026-03-22 - Copilot: transitioned `HttpRequest::items()` to `std::string` keys, updated `BasicAuth` item storage to standard text, and pinned the API type in the native utility suite.
2026-03-22 - Copilot: moved `BasicAuth`, `CrossOriginRequestSharing`, and request-handler response factory entry points to standard-text-first APIs, kept Arduino overloads as adapters, and extended native tests for the new front doors.
...
- `src/core/HttpHeader.*`, `src/core/HttpHeaderCollection.*`, and `src/core/HttpRequest.*`: now migrated to `std::string` internal ownership with Arduino `String` adapters retained at the boundary, and `HttpRequest::items()` now uses `std::map<std::string, std::any>` instead of Arduino-string keys. Classification: owned internal state. Preferred replacement: keep `std::string` internally and leave compatibility adapters temporary.
...
- `src/routing/BasicAuthentication.h` and `src/routing/CrossOriginRequestSharing.h`: now expose `std::string_view`-first APIs for auth and CORS configuration while retaining Arduino `String` overloads as boundary adapters. Classification: compatibility overload / Arduino-facing boundary. Preferred replacement: keep only the user-facing adapters that remain useful until a later public API cleanup phase.
