...
namespace HttpServerAdvanced
{
    namespace Compat
    {
        namespace platform
        {
              ...
              public:
                ArduinoFile(FS& fileSystem, File file, std::string fsPath, std::string path, bool directory,
                    ...
                    : fileSystem_(fileSystem), file_(std::move(file)), fsPath_(std::move(fsPath)), path_(std::move(path)),
                      name_(std::string(VirtualPathMapper::BasenameView(path_))), directory_(directory), size_(size),
            ...
            class ArduinoFSAdapter : public IFileSystem
            {
              public:
                explicit ArduinoFSAdapter(FS& fileSystem)
                    : fileSystem_(fileSystem), mapper_(VirtualPathMapper::NormalizeRootPath("/"))
                {
                }

                ArduinoFSAdapter(FS& fileSystem, std::string_view rootPath)
                    : fileSystem_(fileSystem), mapper_(VirtualPathMapper::NormalizeRootPath(rootPath))
                {
                }
              ...
              private:
                                  ...
                                  const DirectoryEntryCallback& callback, const bool recursive)
                {
                    ...
                    while (true)
                    {
                        ...
                        {
                            entryFsPath = VirtualPathMapper::JoinScopedPath(fsDirectoryPath, CopyCString(entry.name()));
                        }

                        const std::string entryVirtualPath = VirtualPathMapper::Join(virtualDirectoryPath, CopyCString(entry.name()));
                        const DirectoryEntry directoryEntry{std::string(VirtualPathMapper::BasenameView(entryVirtualPath)),
