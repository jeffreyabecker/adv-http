...
inline bool EnumerateHostDirectory(std::string_view directoryPath,
                                   Callback &&callback) {
  ...
  while (dirent *current = readdir(directory)) {
    ...
    const std::string entryHostPath =
        VirtualPathMapper::JoinScopedPath(ownedDirectory, name);
...
inline bool CreateHostDirectory(std::string_view /*path*/) { return false; }
