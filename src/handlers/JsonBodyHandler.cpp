...
namespace HttpServerAdvanced
{
    ...
    {
        return [handler](HttpContext &context, RouteParameters &&, JsonDocument &&jsonData)
        {
    ...
    IHttpHandler::Factory Json::makeFactory(Invocation handler, ExtractArgsFromRequest extractor)
    {
        return [handler, extractor](HttpContext &context) -> std::unique_ptr<IHttpHandler>
        {
            ...
            return std::make_unique<JsonBodyHandler>(handler, ExtractArgsFromRequest([params](HttpContext &c)
                                                                                     { return params; }));
        };
    }
    ...
    {
        return [interceptor, handler](HttpContext &context, RouteParameters &&params, JsonDocument &&jsonData)
        {
            return interceptor(context, [handler, params = std::move(params), jsonData = std::move(jsonData)](HttpContext &context) mutable
    ...
    {
        return [filter, handler](HttpContext &context, RouteParameters &&params, JsonDocument &&jsonData)
        {
